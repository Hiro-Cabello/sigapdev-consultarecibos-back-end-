...
public class AlumnoProgramaJOINProgramaJOINAlumnoDAOImpl implements IAlumnoProgramaJOINProgramaJOINAlumnoDAO {
	...
	public List<AlumnoSemestre> getAlumnoSemestre(Integer semestre, String periodoinicial,String periodofinal) {
		...
		String wel = "select distinct (ape_paterno || '|' || ape_materno || '|' || nom_alumno) as nombre_completo, " + 
				...
				"or ((CAST(substring(anio_ingreso,1,4) AS integer) between ? and ?) and substring(anio_ingreso,6,7)!='' AND (CAST(substring(anio_ingreso,6,7) AS integer))<=?)) " + 
				"or (CAST(substring(anio_ingreso,1,4) AS integer) between ? and ?));";
		RowMapper<AlumnoSemestre> rowMapper = new AlumnoSemestreRowMapper();
		Integer year1 = Integer.parseInt(periodoinicial.substring(0, 4));
